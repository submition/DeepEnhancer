...
from basicsr.data.film_dataset import Film_train_dataset, Film_test_dataset
...
class Trainer():
    def __init__(self, config, opts, this_logger, debug=False):
        ...
        self.val_dataset = Film_test_dataset(config['datasets']['val'])
    ...
    def optimize_parameters(self, lq, gt, current_iter):
        ...
        if self.opts.fix_iter > 0:
            if current_iter == 1:
                print("the parameters of spynet will not be updated")
                for k, v in self.netG.named_parameters():
                    if 'spynet' in k:
                        v.requires_grad = False
            elif current_iter == self.opts.fix_iter + 1:
                print("all paramsters will be updated")
    ...
    def validation(self):
        ...
        for val_data in self.val_loader:  ### Once load all frames
            ...
            if self.config['val']['save_img']:
                for id, sr_img in zip(val_data['frame_list'], sr_imgs):
                    save_place = os.path.join(self.config['path']['visualization'], self.config['datasets']['val']['name'],
        ...
        if calculate_metric:
            ...
            self._update_best_metric_result(self.config['datasets']['val']['name'], 'psnr', self.PSNR, self.iteration)
            self._update_best_metric_result(self.config['datasets']['val']['name'], 'ssim', self.SSIM, self.iteration)

            # self.logger.info(log_str)

            self._log_validation_metric_values(self.iteration, self.config['datasets']['val']['name'])

    def validation_any_resolution(self):
        ...
        for val_data in self.val_loader:  ### Once load all frames
            ...
            for i in range(0, all_len, val_frame_num):  ### Fetch parts of the input video
                ...
                current_part['key'] = val_data['key']
                current_part['frame_list'] = val_data['frame_list'][i:min(i + val_frame_num, all_len)]
                part_lq = current_part['lq'].to(self.opts.device)
                ...
                with torch.no_grad():
                    ...
                    part_lq = torch.cat(
                        ...
                        4)[:, :, :, :, :w + w_pad]
                    part_output, _ = self.netG(part_lq)
                    self.part_output = part_output[:, :, :, :h, :w]
        ...
        for epoch in range(self.epoch):
            ...
            for data in self.train_loader:
                ...
                if self.iteration % self.config['logger']['print_freq'] == 0 and self.opts.global_rank == 0:
                    ...
                    log_vars.update({'lr_g': self.old_lr, 'lr_d': self.old_gan_lr})  ## List learning rate: include the lrs of flow and generator
                ...
                if self.iteration % self.config['val']['val_freq'] == 0 and self.opts.global_rank == 0:
                    ...
                    # self.validation_any_resolution()  #cuda out of memory
                    self.validation()
