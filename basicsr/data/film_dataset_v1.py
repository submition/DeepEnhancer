...
from basicsr.data.Data_Degradation.util import degradation_video_colorization,degradation_video_colorization_test, transfer_1, transfer_2
...
class Film_color_train_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_results = rgb2lab(img_results)

        img_lqs = img_results[:self.num_frame]
        img_gts = img_results[self.num_frame:]
...
class Film_color_test_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_lqs, img_gts = degradation_video_colorization_test(img_gts, interval_length)    # a,b channel to 0
        #######################################

        # lq_v = np.clip(img_lqs[1] * 255., 0, 255)
        # lq_v = cv2.cvtColor(lq_v,cv2.COLOR_BGR2GRAY)
        # cv2.imshow('lq.png',lq_v)
        # cv2.waitKey(0)
        ...
        img_results = torch.stack(img_results, dim=0)
        img_results = rgb2lab(img_results)
        img_lqs = img_results[:current_len]
        img_gts = img_results[current_len:]

        # print(img_gts[0, 1:2, 0, 0])
        print(img_lqs[0, 1:2, 0, 0])
...
class Film_color_test_dataset_v1(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_results = torch.stack(img_results, dim=0)
        img_results = rgb2lab(img_results)
        img_lqs = img_results[:,1:3,:,:]

        img_results = []
        for x in img_gts:
            img_results.append(to_mytensor(x))
        img_results = torch.stack(img_results, dim=0)
        img_gts = rgb2lab(img_results)

        # print(img_gts.shape)   t,2,h,w
        # print(img_lqs.shape)   t,3,h,w

        # img_gts = rgb2lab(img_gts)
        # img_lqs = img_gts[:, 0, :, :]
        # print(img_gts)
        # print(img_lqs.shape)
