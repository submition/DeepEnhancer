...
class Film_color_train_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for id in range(all_len):
            if id in keyframe_idx or (id == all_len - 1):
                img_lqs[id] = img_gts[id]
            else:
                img_lqs[id, 0, :, :] = img_gts[id, 0, :, :]
                img_lqs[id, 1:3:, :] = 0.
...
class Film_color_test_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if keyframe_idx[-1] == (all_len - 1):
            keyframe_idx = keyframe_idx[:-1]
        # print("1111:",all_len)
        # print("2222:",keyframe_idx)

        for id in range(all_len):
            # if id in keyframe_idx:
            if id in keyframe_idx or (id == all_len - 1):
                img_lqs[id] = img_gts[id]
            else:
                img_lqs[id, 0, :, :] = img_gts[id, 0, :, :]
