...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if any(i > 0 for i in self.expand_size) and self.focal_level > 0:
            ...
            # print(k_tl_windows.shape, k_tr_windows.shape, k_bl_windows.shape, k_br_windows.shape)
            k_rolled = torch.cat(
            ...
            # print(self.valid_ind_rolled.shape)
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                ...
                #     1, T,self.unfolds[k].kernel_size[0], self.unfolds[k].kernel_size[1], -1)
                # print(unfolded_mask.shape)
                # unfolded_mask = unfolded_mask.permute(4, 1, 2, 3, 0).contiguous().view(nWh * nWw // stride // stride, -1, 1)
                # t = self.unfolds[k](mask.unsqueeze(0))
                # print(t.shape)
...
if __name__ == '__main__':
    input = torch.randn(1,7,20,36,512).cuda()
    net =  TemporalFocalTransformerBlock(dim=512,num_heads=4).cuda()
    output_size = (60, 108)
